        ...
        try {
            ...
            if (result.hasException() && GenericService.class != invoker.getInterface()) {
                try {
                    ...
                    //存在异常时屏蔽掉异常，包装成自定义返回的数据

                    if (exception instanceof WfException) {
                        WfException wfException = (WfException) exception;
                        ApiResult<Object> exceptionRes = ApiResult.fail(wfException.getCode());
                        exceptionRes.setMsg(wfException.getMessage());
                        if (result instanceof RpcResult) {
                            RpcResult rpcResult = (RpcResult) result;
                            rpcResult.setValue(exceptionRes);
